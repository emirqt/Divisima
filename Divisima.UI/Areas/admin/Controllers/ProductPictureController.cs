        ...
        public async Task<IActionResult> Add(ProductPicture model)
        {
            if (ModelState.IsValid) // gelen değer doğrulanmışsa
            {
                if (Request.Form.Files.Any())
                {
                    ...
                    using (FileStream stream = new FileStream(Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "img", "product", dosyaAdi), FileMode.Create))
                    {
                        await Request.Form.Files["Picture"].CopyToAsync(stream);
                    }
                    model.Picture = "/img/product/" + dosyaAdi;
                }
